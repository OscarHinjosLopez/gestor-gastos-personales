import { Component, Input, ViewChild, ElementRef, OnInit, OnDestroy, OnChanges, SimpleChanges, AfterViewInit, inject, ChangeDetectionStrategy } from '@angular/core';
...
@Component({
  ...
  template: `
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6" 
      ...
      <div class="relative">
        @if (isLoading) {
          <div class="w-full h-80 flex items-center justify-center bg-gray-50 rounded-lg">
          ...
          <div class="w-full h-80 flex items-center justify-center bg-red-50 rounded-lg border border-red-200">
        ...
        @if (showSummary && !isLoading && !hasError) {
          <div class="mt-4 grid grid-cols-3 gap-4 text-sm">
            <div class="bg-gray-50 rounded-lg p-3">
              <div class="text-gray-600 text-xs uppercase tracking-wide">{{ label1 }}</div>
              <div class="text-lg font-semibold" 
                   [attr.aria-label]="label1 + ': ' + (value1 | currency:'EUR':'symbol':'1.0-0')">
                {{ value1 | currency:'EUR':'symbol':'1.0-0' }}
              </div>
            </div>
            <div class="bg-gray-50 rounded-lg p-3">
              <div class="text-gray-600 text-xs uppercase tracking-wide">{{ label2 }}</div>
              <div class="text-lg font-semibold"
                   [attr.aria-label]="label2 + ': ' + (value2 | currency:'EUR':'symbol':'1.0-0')">
...
export class BasicChartComponent implements AfterViewInit, OnDestroy, OnChanges {
  @Input() title: string = '';
  @Input() icon: string = '';
        ...
        tooltip: {
          callbacks: {
            label: (context: any) => {
              ...
              return `${context.label}: €${value.toLocaleString()} (${percentage > 0 ? '+' : ''}${percentage}%)`;
            }
          }
        }
      },
  ...
  getChartAriaLabel(): string {
    ...
    return `Gráfico de barras mostrando ${this.title}. ${this.label1}: €${this.value1}, ${this.label2}: €${this.value2}. ${trend} ${Math.abs(percentage)}%`;
  }
