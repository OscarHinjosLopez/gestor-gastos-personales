...
export class PeriodComparisonService {
   ...
   */
  async comparePeriods(period1: DateRange, period2: DateRange, filter?: ComparisonFilter): Promise<PeriodComparison> {
   ...
   */
  private calculatePeriodData(range: DateRange, filter?: ComparisonFilter): PeriodData {
    ...
    const uniqueDaysWithTransactions = this.getUniqueDaysWithTransactions(expenses, incomes);
    ...
    return {
      ...
      categoryBreakdown,
      sourceBreakdown,
      topCategory: categoryBreakdown.length > 0 ? categoryBreakdown[0] : undefined,
      topSource: sourceBreakdown.length > 0 ? sourceBreakdown[0] : undefined,
      daysWithTransactions: uniqueDaysWithTransactions,
      savingsRate: totalIncomes > 0 ? ((totalIncomes - totalExpenses) / totalIncomes) * 100 : 0
    };
  }
  ...
  private filterTransactionsByDateAndCriteria(
    ...
    if (type === 'expense' && filter.categories && filter.categories.length > 0) {
   ...
   */
  private calculateComparisonMetrics(period1Data: PeriodData, period2Data: PeriodData): ComparisonMetrics {
    return {
      incomeDelta: this.calculateDelta(period1Data.totalIncomes, period2Data.totalIncomes),
      expenseDelta: this.calculateDelta(period1Data.totalExpenses, period2Data.totalExpenses),
      balanceDelta: this.calculateDelta(period1Data.balance, period2Data.balance),
      transactionCountDelta: this.calculateDelta(period1Data.transactionCount, period2Data.transactionCount),
      averageDailyExpenseDelta: this.calculateDelta(period1Data.averageDailyExpense, period2Data.averageDailyExpense),
      averageDailyIncomeDelta: this.calculateDelta(period1Data.averageDailyIncome, period2Data.averageDailyIncome),
      savingsRateDelta: this.calculateDelta(period1Data.savingsRate, period2Data.savingsRate),
      categoryChanges: this.calculateCategoryChanges(period1Data.categoryBreakdown, period2Data.categoryBreakdown),
      sourceChanges: this.calculateSourceChanges(period1Data.sourceBreakdown, period2Data.sourceBreakdown)
    };
  }
   ...
   */
  private calculateCategoryChanges(current: CategoryData[], previous: CategoryData[]): CategoryComparison[] {
   ...
   */
  private calculateSourceChanges(current: SourceData[], previous: SourceData[]): SourceComparison[] {
    ...
    return Array.from(allSources).map(sourceName => {
   ...
   */
  private generateInsights(period1Data: PeriodData, period2Data: PeriodData, metrics: ComparisonMetrics): ComparisonInsight[] {
    ...
    if (metrics.balanceDelta.significance === 'high') {
      insights.push({
        ...
        category: 'balance',
        title: metrics.balanceDelta.trend === 'up' ? 'Mejora significativa del balance' : 'Deterioro significativo del balance',
        description: `Tu balance ${metrics.balanceDelta.trend === 'up' ? 'mejoró' : 'empeoró'} en ${Math.abs(metrics.balanceDelta.percentage).toFixed(1)}% comparado con el período anterior`,
    ...
    if (metrics.incomeDelta.significance === 'high') {
      insights.push({
        ...
        category: 'income',
        title: metrics.incomeDelta.trend === 'up' ? 'Aumento considerable de ingresos' : 'Reducción considerable de ingresos',
        description: `Tus ingresos ${metrics.incomeDelta.trend === 'up' ? 'aumentaron' : 'disminuyeron'} ${Math.abs(metrics.incomeDelta.percentage).toFixed(1)}%`,
    ...
    if (metrics.expenseDelta.significance === 'high') {
      insights.push({
        ...
        category: 'expense',
        title: metrics.expenseDelta.trend === 'up' ? 'Aumento significativo de gastos' : 'Reducción significativa de gastos',
        description: `Tus gastos ${metrics.expenseDelta.trend === 'up' ? 'aumentaron' : 'disminuyeron'} ${Math.abs(metrics.expenseDelta.percentage).toFixed(1)}%`,
    ...
    if (Math.abs(metrics.savingsRateDelta.percentage) > 15) {
      insights.push({
        ...
        category: 'behavior',
        title: `${metrics.savingsRateDelta.trend === 'up' ? 'Mejora' : 'Reducción'} en tasa de ahorro`,
        description: `Tu tasa de ahorro ${metrics.savingsRateDelta.trend === 'up' ? 'mejoró' : 'empeoró'} ${Math.abs(metrics.savingsRateDelta.absolute).toFixed(1)} puntos porcentuales`,
    ...
    const significantCategoryChanges = metrics.categoryChanges.filter(c => c.delta.significance === 'high');
    significantCategoryChanges.slice(0, 2).forEach(change => {
      insights.push({
        ...
        description: `Gastos en ${change.name} ${change.delta.trend === 'up' ? 'aumentaron' : 'disminuyeron'} ${Math.abs(change.delta.percentage).toFixed(1)}%`,
    ...
    if (newCategories.length > 0) {
      insights.push({
        ...
        description: `Aparecieron ${newCategories.length} nueva(s) categoría(s): ${newCategories.map(c => c.name).join(', ')}`,
        priority: 'low'
      });
    }
   ...
   */
  private getUniqueDaysWithTransactions(expenses: Expense[], incomes: Income[]): number {
  ...
  getAvailableSources(): string[] {
    ...
    const sources = [...new Set(incomes.map(i => i.source).filter(Boolean) as string[])];
    return sources.sort();
  }
}
